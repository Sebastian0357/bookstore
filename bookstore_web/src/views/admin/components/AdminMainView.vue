<template>
  ...
  export default {
    name: "AdminIndexView",
    components: { AdminAsideView, AdminHeaderView },
    data() {
      return {
        isCollapse: false,
        asideWidth: '200px',
        icon: 'bi bi-caret-right-fill' // 使用 Bootstrap 图标
      };
    },
    methods: {
      doCollapse() {
        ...
        if (!this.isCollapse) {
          ...
          this.icon = 'bi bi-caret-right-fill'; // 展开
        } else {
          this.asideWidth = '64px';
          this.icon = 'bi bi-caret-left-fill'; // 收起
        }
      }
    }
  };
